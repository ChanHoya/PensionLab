...
export async function POST(request: Request) {
  try {
    ...
    if (openai) {
      ...
      // Fallback: SQL keyword ILIKE search if OpenAI key is not configured
      ...
      answer = `⚠️ **[로컬 테스트 모드 - OpenAI API Key 미등록 상태]**\n\n질문과 관련된 데이터베이스 매칭 전문가 소견을 안내해 드립니다:\n\n${summaryText}\n\n자세한 상담을 위해 향후 OpenAI API 키를 설정하여 실시간 대화형 RAG 기능을 활성화하실 수 있습니다.`;
    }
